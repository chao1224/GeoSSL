...
class NCSN_version_01(torch.nn.Module):
    def __init__(self, emb_dim, sigma_begin, sigma_end, num_noise_level, noise_type, anneal_power):
        super(NCSN_version_01, self).__init__()

        self.anneal_power = anneal_power

        self.noise_type = noise_type
        self.output_mlp = MultiLayerPerceptron(emb_dim, [emb_dim, emb_dim// 2, 1])
    ...
    def forward(self, data, node_feature, distance, debug=False):
        ...
        distance_feature = torch.cat([h_row*h_col], dim=-1) # (num_edge, hidden)
...
class NCSN_version_02(torch.nn.Module):
    def __init__(self, emb_dim, sigma_begin, sigma_end, num_noise_level, noise_type, anneal_power):
        super(NCSN_version_02, self).__init__()
        ...
        self.input_distance_mlp = MultiLayerPerceptron(num_gaussians, [emb_dim, emb_dim], activation="relu")
        self.output_mlp = MultiLayerPerceptron(2 * emb_dim, [emb_dim, emb_dim// 2, 1])
    ...
    def forward(self, data, node_feature, distance, debug=False):
        ...
        perturbed_distance = distance + distance_noise * used_sigmas
        distance_emb = self.distance_expansion(perturbed_distance) # (num_edge, num_gaussians)
        distance_emb = self.input_distance_mlp(distance_emb) # (num_edge, hidden)
