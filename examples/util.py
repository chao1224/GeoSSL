...
def energy_mae(pred_Es, Es, **kwargs):
    return F.l1_loss(pred_Es, Es, reduction='sum')

def force_mae(pred_Fs, Fs, fixed_masks, **kwargs):
    N_free_atoms = torch.sum(fixed_masks, dim=-1, keepdim=True)
    loss_F = F.l1_loss(pred_Fs, Fs, reduction='none').sum(dim=-1)
    loss_F = (loss_F / N_free_atoms).masked_select(fixed_masks.bool()).sum()
    return loss_F

def force_cosine(pred_Fs, Fs, fixed_masks, **kwargs):
    _ret = F.cosine_similarity(pred_Fs, Fs, dim=-1)
    N_free_atoms = torch.sum(fixed_masks, dim=-1, keepdim=True)
    _ret = (_ret / N_free_atoms).masked_select(fixed_masks.bool()).sum()
    return _ret

def EwT(pred_Es, Es, epsilon=0.02, **kwargs):
    """
    Args:
        pred_Es: (B, ), fl
        Es: (B, ), fl
    """
    return torch.mean((torch.abs(pred_Es - Es) < epsilon).float())
    
def EFwT(pred_Es, Es, pred_Fs, Fs, epsilon=0.02, alpha=0.03, **kwargs):
    """
    Args:
        pred_Es: (B, ), fl
        Es: (B, ), fl
        pred_Fs: (B, N_max_atom), fl
        Fs: (B, N_max_atom), fl
    """
    E_mask = torch.abs(pred_Es - Es) < epsilon
    loss_F = F.l1_loss(pred_Fs, Fs, reduction='none').sum(dim=-1)
    F_mask = torch.abs(torch.max(loss_F, dim=-1)[0]) < alpha
    return torch.mean((E_mask & F_mask).float())