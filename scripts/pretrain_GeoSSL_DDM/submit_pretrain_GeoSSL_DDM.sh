cd ../../examples
export mode=pretrain_GeoSSL_SM
...
export GeoSSL_sigma_list=(0.3 1 5)
...
for SM_noise_type in "${SM_noise_type_list[@]}"; do
    ...
    export output_model_dir=../output/"$mode"/"$dataset"_"$model_3d"/"$lr"_"$lr_scheduler"_"$GeoSSL_mu"_"$GeoSSL_sigma"_"$GeoSSL_atom_masking_ratio"_"$GeoSSL_option"_"$SM_noise_type"_"$epochs"
    ...
    if [[ ! -f "$output_model_dir"/model_final.pth ]]; then
        ...
        sbatch --gres=gpu:v100l:1 -c 8 --mem=32G -t "$time":59:00  --account=rrg-bengioy-ad --qos=high --job-name=SM_"$model_3d"_"$time" \
        --output="$output_file" \
        ./run_"$mode".sh \
