...
for GeoSSL_option in "${GeoSSL_option_list[@]}"; do
    ...
    export output_model_dir=../output/"$mode"/"$dataset"_"$model_3d"/"$lr"_"$lr_scheduler"_"$GeoSSL_mu"_"$GeoSSL_sigma"_"$GeoSSL_atom_masking_ratio"_"$GeoSSL_option"_"$epochs"
    export output_file="$output_model_dir"/result.out
    echo "$output_model_dir"
    mkdir -p "$output_model_dir"

    if [[ ! -f "$output_file" ]]; then
        sbatch --gres=gpu:v100l:1 -c 8 --mem=32G -t "$time":59:00  --account=rrg-bengioy-ad --qos=high --job-name=RR_"$model_3d"_"$time" \
        --output="$output_file" \
        ...
        --GeoSSL_mu="$GeoSSL_mu" --GeoSSL_sigma="$GeoSSL_sigma" \
        --GeoSSL_atom_masking_ratio="$GeoSSL_atom_masking_ratio" --GeoSSL_option="$GeoSSL_option" --normalize \
