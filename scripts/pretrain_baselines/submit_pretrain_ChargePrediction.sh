cd ../../examples
export mode=pretrain_ChargePrediction
...
export charge_masking_ratio_list=(0.15 0.3)
...
for charge_masking_ratio in "${charge_masking_ratio_list[@]}"; do

    export output_model_dir=../output/"$mode"/"$dataset"_"$model_3d"/"$lr"_"$lr_scheduler"_"$charge_masking_ratio"_"$epochs"
    export output_file="$output_model_dir"/result.out
    echo "$output_model_dir"
    mkdir -p "$output_model_dir"

    if [[ ! -f "$output_file" ]]; then
        sbatch --gres=gpu:v100l:1 -c 8 --mem=32G -t "$time":59:00  --account=rrg-bengioy-ad --qos=high --job-name=CP_"$model_3d"_"$time" \
        --output="$output_file" \
        ./run_"$mode".sh \
        --model_3d="$model_3d" --input_data_dir="$input_data_dir" --dataset="$dataset" \
        --epochs="$epochs" --lr="$lr" --lr_scheduler="$lr_scheduler" \
        --charge_masking_ratio="$charge_masking_ratio" \
