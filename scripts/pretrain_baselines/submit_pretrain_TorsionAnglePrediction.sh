cd ../../examples
export mode=pretrain_TorsionAnglePrediction
...
export torsion_angle_sample_ratio_list=(1e-3 1e-4)
...
for torsion_angle_sample_ratio in "${torsion_angle_sample_ratio_list[@]}"; do

    export output_model_dir=../output/"$mode"/"$dataset"_"$model_3d"/"$lr"_"$lr_scheduler"_"$torsion_angle_sample_ratio"_"$epochs"
    ...
    if [[ ! -f "$output_file" ]]; then
        ...
        --torsion_angle_sample_ratio="$torsion_angle_sample_ratio" \
