...
from Geom3D.datasets.datasets_utils import allowable_features
...
class TransformLBA:
    ...
    def _replace(self, df, keep=["H", "C", "N", "O", "F", "S", "P", "S", "Cl"], new="Cu"):
        new_elements = []
...
class DatasetLBA(InMemoryDataset):
    ...
    def process(self):
        ...
        print("len of structured_dict\t", len(structure_dict))
        ...
        for data_id, (pdb_id, data) in enumerate(tqdm(structure_dict.items(), desc="creating pyg data")):
            ...
            for ia in range(len(merged_df)):
                ...
                if atomic_number not in allowable_features["possible_atomic_num_list"]:
                    atomic_number = "unknown"
                atom_features = allowable_features["possible_atomic_num_list"].index(atomic_number)
                x = merged_df['x'][ia]
                y = merged_df['y'][ia]
                z = merged_df['z'][ia]
